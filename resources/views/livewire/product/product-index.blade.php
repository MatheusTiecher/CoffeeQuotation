    ...
    <table class="table-auto w-full mx-auto">
        <thead>
            <tr class="text-left">
                ...
                <th class="px-4 py-2">Preço Padrão</th>
        ...
        <tbody>
            @foreach ($products as $product)
                <tr>
                    ...
                    <td class="px-4 py-2 border">{{ $product->price_default }}</td>
                    <td class="px-4 py-2 border">{{ $product->created_at->format('d/m/Y H:i:s') }}</td>
                    <td class="px-4 py-4 border">
                        <a href="{{ route('products.edit', $product->id) }}"
                            class="px-4 py-2 border rounded bg-blue-500 text-white">Editar</a>
                        <a href="#" wire:click.prevent="remove({{ $product->id }})"
